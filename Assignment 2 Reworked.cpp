...
#define reward 1000
int multiplier=0;
struct player_details{ //this is basically our data structure for Player and points.
	string player_name;
	int player_ID;
	long points; //long is doubling the bits of integer, basically long is 32 bits while int is 16.
}player[2];

//My Knowns that'll use to make this program works later on.
int Logic( int, int);
//Why do we use void?, we did use void so our functions doesn't have to return values and mess everything up.
//what if we used integer?, We'll have lots of values printed together and our program will be messed up.
...
int main()
{
	system ("color 0e"); //personanl customization
	int choice, choiceby1, choiceby2, winner;
	bool x, locker; //bool used here so it can check if our function, data is true or false and is a must for our initializing, To control our true and false outputs.
	//And the return for the size of bool is always 1, unlike int which is 4.
	//register the user
	...
	Sleep(1); // Sleep functions in delaying our program abit here.
	player[0].points=10000; //declaring start points for each player.
	player[1].points=10000;

	do{ // we usue do _while loop to make sure each loop works atleast once.
		...
		do{
			...
			//this is to make sure that the choice entered by player is correct and also checks it it's wrong.
			...
			if(winner==0||winner==2) {
				...
				getch(); //getch function is used to catch a character from the keyboard.
				}//means generating interaction therefore play again but now for more additional points.

		}while(winner==10);


		do{ //do loop with bool functioning inside.
			locker=false;
...
//elements list.
void Element_displayer(){
	system("CLS"); // CLS is a command to Clear screen, But won't erase data.
...
//Logic in here is the Winner determining.
...
// My function for points calculation and results.
// Points list and Winner declaration aswell.
